...
public class GSVDatasetWriter extends DatasetWriter {
    ...
    public static int writeAttack(ProtectionIED legitimateIED, ProtectionIED attacker, MergingUnit mu, boolean printHeader) throws IOException {
        ...
        for (int i = 0; i < attacker.getMessages().size(); i++) {
            if (i <= ConfigLoader.gooseFlow.numberOfMessages) {
                ...
                if (br.ufu.facom.ereno.dataExtractors.DatasetWriter.Debug.gooseMessages) {
                    System.out.println("------------");
                ...
                Logger.getLogger("Util").warning("Skipping " + (attacker.getNumberOfMessages() - ConfigLoader.gooseFlow.numberOfMessages) + " malicious messages to make it balanced with normal class.");
                return i;
            }
        }
    ...
    private static void writeToDataset(ArrayList<Sv> svMessages, Goose prev, Goose attack, Sv sv) throws IOException {
        ...
        double e2eLatency = attack.getE2ELatencyMs();
        double receivedTimestamp = attack.getSubscriberRxTs() != null ? attack.getSubscriberRxTs() : attack.getTimestamp();
        write(svString + "," + cycleStrig + "," + gooseString + "," + delay + "," + e2eLatency + "," + receivedTimestamp + "," + attack.getLabel());
