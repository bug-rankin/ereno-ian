...
public class SamambaiaScenario implements IScenario {
    ...
    public void loadAllConfigs() {
        ...
        // scenario-specific overrides via ConfigLoader
        ConfigLoader.attacks.legitimate = true;
        ConfigLoader.attacks.randomReplay = true;
        ConfigLoader.attacks.masqueradeOutage = false;
        ConfigLoader.attacks.masqueradeDamage = false;
        ConfigLoader.attacks.randomInjection = false;
...
//        substationNetwork.bayLevelDevices.add(uc08);
    ...
    public void runDevices() {
        ...
        for (MergingUnit mu : substationNetwork.processLevelDevices) {
            mu.run(br.ufu.facom.ereno.config.ConfigLoader.gooseFlow.numberOfMessages); // Do not repeat SV now, delegate it to the generation step
//            mu.run(numberOfMessages * 4763); // For each GOOSE message, there are about to 4800 SV messages
        ...
        for (IED ied : substationNetwork.bayLevelDevices) {
            ...
            ied.run(br.ufu.facom.ereno.config.ConfigLoader.gooseFlow.numberOfMessages);
            int numAddedMessages = 0;

            for (Goose goose : ((ProtectionIED) ied).getMessages()) {
                numAddedMessages++;
                if (numAddedMessages < br.ufu.facom.ereno.config.ConfigLoader.gooseFlow.numberOfMessages) {
